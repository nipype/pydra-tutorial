...
#
# Task might be run for a single set of input values or we can generate multiple sets, this will be called "states". If we want to run our `Task` multiple times we have to provide an input that is an iterable and specify the way we want to map values of the inputs to the specific states. In order to do this, we set the so-called `splitter`.
#
...
# Note, that *pydra* adds the name of the function to the name of the input.
#
# Now, we can run the task and check its results:
...
# ![nd_spl_2.png](attachment:./figures/nd_spl_2.png)

# We can also have lists for both inputs, and use both inputs in the splitter. Let's assume that `a` and `b` are lists with two elements each.
...
# ![nd_spl_4.png](attachment:./figures/nd_spl_4.png)
...
# ![nd_spl_3.png](attachment:./figures/nd_spl_3.png)
...
# For more inputs we can create a more complex splitter, and use scalar and outer splitters together. **Note, that the scalar splitter can only work for lists that have the same length, but the outer splitter doesn't have this limitation.**
#
# Let's run one more example that takes four inputs, `x` and `y` components of two vectors, and calculates all possible sums of vectors. `x` components should be kept together with corresponding `y` components (i.e. scalar splitters: `("x1", "y1")` and `("x2", "y2")`), but we should use the outer splitter for two vectors to get all combinations.
...
#
# When we use `splitter`, we can also define `combiner`, if we want to combine the results together.
#
...
# ![nd_spl_3_comb1.png](attachment:./figures/nd_spl_3_comb1.png)
...
# ![nd_spl_3_comb2.png](attachment:./figures/nd_spl_3_comb2.png)
...
# ![nd_spl_3_comb3.png](attachment:./figures/nd_spl_3_comb3.png)
...
# ### Parallel execution
#
# We run the task multiple times for multiple sets of input, but we didn't talk about the execution time. Let's create a function that sleeps for a second and then runs for four values:
